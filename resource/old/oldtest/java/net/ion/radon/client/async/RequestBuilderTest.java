...
import net.ion.radon.client.FluentStringsMap;
...
public class RequestBuilderTest {
	...
	public void testEncodesQueryParameters() throws UnsupportedEncodingException {
		String[] values = new String[] { "abcdefghijklmnopqrstuvwxyz", "ABCDEFGHIJKQLMNOPQRSTUVWXYZ", "1234567890", "1234567890", "`~!@#$%^&*()", "`~!@#$%^&*()", "_+-=,.<>/?", "_+-=,.<>/?", ";:'\"[]{}\\| ", ";:'\"[]{}\\| " };

		/*
		 * as per RFC-5849 (Oauth), and RFC-3986 (percent encoding) we MUST encode everything except for "safe" characters; and nothing but them. Safe includes ascii letters (upper and lower case), digits (0 - 9) and FOUR special characters: hyphen ('-'), underscore ('_'), tilde ('~') and period ('.')). Everything else must be percent-encoded, byte-by-byte, using UTF-8 encoding (meaning three-byte
		 * Unicode/UTF-8 code points are encoded as three three-letter percent-encode entities).
		 */
		for (String value : values) {
			RequestBuilder builder = new RequestBuilder("GET").setUrl("http://example.com/").addQueryParameter("name", value);
	...
	public void testChaining() throws IOException, ExecutionException, InterruptedException {
		Request request = new RequestBuilder("GET").setUrl("http://foo.com").addQueryParameter("x", "value").build();
	...
	public void testParsesQueryParams() throws IOException, ExecutionException, InterruptedException {
		Request request = new RequestBuilder("GET").setUrl("http://foo.com/?param1=value1").addQueryParameter("param2", "value2").build();
