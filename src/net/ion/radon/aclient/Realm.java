...
public class Realm {
	...
	public String toString() {
		return "Realm{" + "principal='" + principal + '\'' + ", password='" + password + '\'' + ", scheme=" + scheme + ", realmName='" + realmName + '\'' + ", nonce='" + nonce + '\'' + ", algorithm='" + algorithm + '\'' + ", response='" + response + '\'' + ", qop='" + qop + '\'' + ", nc='" + nc
				+ '\'' + ", cnonce='" + cnonce + '\'' + ", uri='" + uri + '\'' + ", methodName='" + method + '\'' + '}';
	}
	...
	public static class RealmBuilder {
		...
		private void newResponse() throws UnsupportedEncodingException {
			...
			md.update(new StringBuilder(principal).append(":").append(realmName).append(":").append(password).toString().getBytes("ISO-8859-1"));
			byte[] ha1 = md.digest();

			md.reset();

			// HA2 if qop is auth-int is methodName:url:md5(entityBody)
			md.update(new StringBuilder(method.getName()).append(':').append(uri).toString().getBytes("ISO-8859-1"));
			byte[] ha2 = md.digest();

			if (qop == null || qop.equals("")) {
				md.update(new StringBuilder(toBase16(ha1)).append(':').append(nonce).append(':').append(toBase16(ha2)).toString().getBytes("ISO-8859-1"));

			} else {
				// qop ="auth" or "auth-int"
				md.update(new StringBuilder(toBase16(ha1)).append(':').append(nonce).append(':').append(NC).append(':').append(cnonce).append(':').append(qop).append(':').append(toBase16(ha2)).toString().getBytes("ISO-8859-1"));
			}
		...
		public Realm build() {
			...
			return new Realm(scheme, principal, password, realmName, nonce, algorithm, response, qop, nc, cnonce, uri, method, usePreemptive, domain, enc, host, messageType2Received, opaque);
		}
	}
