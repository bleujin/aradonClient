...
import net.ion.radon.aclient.generators.InputStreamBodyGenerator;
import net.ion.radon.aclient.listener.TransferCompletionHandler;
import net.ion.radon.aclient.multipart.MultipartBody;
...
import net.ion.radon.aclient.util.CleanupChannelGroup;
...
public class NettyProvider extends SimpleChannelUpstreamHandler implements AsyncHttpProvider {
	...
	private final NettyProviderConfig asyncHttpProviderConfig;
	...
	public NettyProvider(ClientConfig config) {

		if (config.getAsyncHttpProviderConfig() != null && NettyProviderConfig.class.isAssignableFrom(config.getAsyncHttpProviderConfig().getClass())) {
			asyncHttpProviderConfig = NettyProviderConfig.class.cast(config.getAsyncHttpProviderConfig());
		} else {
			asyncHttpProviderConfig = new NettyProviderConfig();
		}

		if (asyncHttpProviderConfig.getProperty(NettyProviderConfig.USE_BLOCKING_IO) != null) {
			socketChannelFactory = new OioClientSocketChannelFactory(config.executorService());
		} else {
			ExecutorService e;
			Object o = asyncHttpProviderConfig.getProperty(NettyProviderConfig.BOSS_EXECUTOR_SERVICE);
	...
	void configureNetty() {
		...
		plainBootstrap.setPipelineFactory(new ChannelPipelineFactory() {
			...
			public ChannelPipeline getPipeline() throws Exception {
				...
				pipeline.addLast("httpProcessor", NettyProvider.this);
				return pipeline;
			}
		});
		...
		if (asyncHttpProviderConfig != null) {
			Object value = asyncHttpProviderConfig.getProperty(NettyProviderConfig.EXECUTE_ASYNC_CONNECT);
			if (value != null && Boolean.class.isAssignableFrom(value.getClass())) {
				executeConnectAsync = Boolean.class.cast(value);
			} else if (asyncHttpProviderConfig.getProperty(NettyProviderConfig.DISABLE_NESTED_REQUEST) != null) {
	...
	protected final <T> void writeRequest(final Channel channel, final ClientConfig config, final NettyResponseFuture<T> future, final HttpRequest nettyRequest) {
		try {
			/**
			...
			Body body = null;
			if (!future.getNettyRequest().getMethod().equals(Method.CONNECT)) {
	...
	private static boolean isProxyServer(ClientConfig config, Request request) {
		return request.getProxyServer() != null || config.getProxyServer() != null;
	}

	protected final static HttpRequest buildRequest(ClientConfig config, Request request, URI uri, boolean allowConnect, ChannelBuffer buffer) throws IOException {

		Method method = request.getMethod();
		if (allowConnect && (isProxyServer(config, request) && isSecure(uri))) {
			method = Method.CONNECT;
		}
		return construct(config, request, method, uri, buffer);
	}

	private static HttpRequest construct(ClientConfig config, Request request, Method m, URI uri, ChannelBuffer buffer) throws IOException {
		...
		if (m.equals(Method.CONNECT)) {
			nettyRequest = new DefaultHttpRequest(HttpVersion.HTTP_1_0, HttpMethod.valueOf(m.getName()), AsyncHttpProviderUtils.getAuthority(uri));
		} else {
			...
			nettyRequest = new DefaultHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.valueOf(m.getName()), path.toString());
		}
		...
		if (request.getHeaders().getFirstValue("User-Agent") != null) {
			...
			nettyRequest.setHeader("User-Agent", AsyncHttpProviderUtils.constructUserAgent(NettyProvider.class));
		}
		...
		if (!m.equals(Method.CONNECT)) {
			...
			Method reqType = request.getMethod();
			if (!Method.GET.equals(reqType) && !Method.HEAD.equals(reqType) && !Method.OPTIONS.equals(reqType) && !Method.TRACE.equals(reqType)) {
...
//			spnegoEngine = new SpnegoEngine();
	...
	public void close() {
		...
		try {
			...
			for (Channel channel : openChannels) {
				ChannelHandlerContext ctx = channel.getPipeline().getContext(NettyProvider.class);
	...
	private <T> ListenableFuture<T> doConnect(final Request request, final AsyncHandler<T> asyncHandler, NettyResponseFuture<T> f, boolean useCache, boolean asyncConnect, boolean reclaimCache) throws IOException {
		...
		if (f != null && f.getRequest().getFile() == null && !f.getNettyRequest().getMethod().getName().equals(Method.CONNECT.getName())) {
		...
		if (!reclaimCache && !connectionsPool.canCacheConnection()) {
			...
			try {
				...
				Debug.warn("!connectionsPool.canCacheConnection()", t);
			}
			throw ex;
		}
		...
		if (System.getProperty("os.name").toLowerCase().indexOf("win") == -1) {
			bootstrap.setOption("reuseAddress", asyncHttpProviderConfig.getProperty(NettyProviderConfig.REUSE_ADDRESS));
		}
		...
		if (directInvokation && !asyncConnect && request.getFile() == null) {
			...
			try {
				...
				try {
					...
					Debug.warn("c.operationComplete()", t);
				}
				throw ioe;
			}
		} else {
	...
	protected static int requestTimeout(ClientConfig config, PerRequestConfig perRequestConfig) {
		int result;
	...
	private Realm kerberosChallenge(List<String> proxyAuth, Request request, ProxyServer proxyServer, FluentCaseInsensitiveStringsMap headers, Realm realm, NettyResponseFuture<?> future) throws NTLMEngineException {
		...
		try {
			...
			return realmBuilder.setUri(uri.getPath()).setMethod(request.getMethod()).setScheme(Realm.AuthScheme.KERBEROS).build();
	...
	private Realm ntlmChallenge(List<String> wwwAuth, Request request, ProxyServer proxyServer, FluentCaseInsensitiveStringsMap headers, Realm realm, NettyResponseFuture<?> future) throws NTLMEngineException {
		...
		if (realm != null && !realm.isNtlmMessageType2Received()) {
			...
			newRealm = new Realm.RealmBuilder().clone(realm).setScheme(realm.getAuthScheme()).setUri(URI.create(request.getUrl()).getPath()).setMethod(request.getMethod()).setNtlmMessageType2Received(true).build();
			...
			newRealm = realmBuilder.setScheme(authScheme).setUri(URI.create(request.getUrl()).getPath()).setMethod(request.getMethod()).build();
		}
	...
	private Realm ntlmProxyChallenge(List<String> wwwAuth, Request request, ProxyServer proxyServer, FluentCaseInsensitiveStringsMap headers, Realm realm, NettyResponseFuture<?> future) throws NTLMEngineException {
		...
		newRealm = realmBuilder// .setScheme(realm.getAuthScheme())
				.setUri(URI.create(request.getUrl()).getPath()).setMethod(request.getMethod()).build();
	...
	private void abort(NettyResponseFuture<?> future, Throwable t) {
		...
		if (channel != null && openChannels.contains(channel)) {
			closeChannel(channel.getPipeline().getContext(NettyProvider.class));
	...
	public void channelClosed(ChannelHandlerContext ctx, ChannelStateEvent e) throws Exception {
		...
		try {
			...
			Debug.warn("super.channelClosed", ex);
		}
	...
	protected boolean remotelyClosed(Channel channel, NettyResponseFuture<?> future) {
		...
		if (future == null && channel.getPipeline().getContext(NettyProvider.class).getAttachment() != null && NettyResponseFuture.class.isAssignableFrom(channel.getPipeline().getContext(NettyProvider.class).getAttachment().getClass())) {
			future = (NettyResponseFuture<?>) channel.getPipeline().getContext(NettyProvider.class).getAttachment();
		}
	...
	private void finishUpdate(final NettyResponseFuture<?> future, final ChannelHandlerContext ctx, boolean lastValidChunk) throws IOException {
		if (lastValidChunk && future.getKeepAlive()) {
			...
			if (future.getKeepAlive() && ctx.getChannel().isReadable() && connectionsPool.offer(AsyncHttpProviderUtils.getBaseUrl(future.getURI()), ctx.getChannel())) {
	...
	public static <T> NettyResponseFuture<T> newFuture(URI uri, Request request, AsyncHandler<T> asyncHandler, HttpRequest nettyRequest, ClientConfig config, NettyProvider provider) {

		NettyResponseFuture<T> f = new NettyResponseFuture<T>(uri, request, asyncHandler, nettyRequest, requestTimeout(config, request.getPerRequestConfig()), config.getIdleConnectionTimeoutInMs(), provider);
		...
		public void operationComplete(ChannelFuture cf) {
			...
			if (cause != null && future.getState() != NettyResponseFuture.STATE.NEW) {
				...
				if (ClosedChannelException.class.isAssignableFrom(cause.getClass()) || abortOnReadCloseException(cause) || abortOnWriteCloseException(cause)) {
					...
					Debug.debug(cf.getCause() == null ? "" : cf.getCause().getMessage(), cf.getCause());
			 ...
			 */
			Realm realm = future.getRequest().getRealm() != null ? future.getRequest().getRealm() : NettyProvider.this.getConfig().getRealm();
	 ...
	 * timeout period which can be arbitrary long.
	 */
	private final class ReaperFuture implements Future, Runnable {
		...
		public synchronized void run() {
			...
			if (nettyResponseFuture != null && nettyResponseFuture.hasExpired() && !nettyResponseFuture.isDone() && !nettyResponseFuture.isCancelled()) {
	...
	private static final boolean validateWebSocketRequest(Request request, AsyncHandler<?> asyncHandler) {
		if ((!request.getMethod().equals(Method.GET)) || !WebSocketUpgradeHandler.class.isAssignableFrom(asyncHandler.getClass())) {
			return false;
		}
		return true;
	}
			...
			if (future.incrementAndGetCurrentRedirectCount() < config.getMaxRedirects()) {
				...
				if (!uri.toString().equalsIgnoreCase(future.getURI().toString())) {
					...
					AsyncCallable ac = new AsyncCallable(future) {
						public Object call() throws Exception {
							if (initialConnectionKeepAlive && ctx.getChannel().isReadable() && connectionsPool.offer(AsyncHttpProviderUtils.getBaseUrl(initialConnectionUri), ctx.getChannel())) {
								return null;
							}
							finishChannel(ctx);
							return null;
		...
		public void handle(final ChannelHandlerContext ctx, final MessageEvent e) throws Exception {
			...
			try {
				if (e.getMessage() instanceof HttpResponse) {
					...
					HttpResponseStatus status = new ResponseStatus(future.getURI(), response, NettyProvider.this);
					HttpResponseHeaders responseHeaders = new ResponseHeaders(future.getURI(), response, NettyProvider.this);
					...
					if (statusCode == 401 && wwwAuth.size() > 0 && !future.getAndSetAuth(true)) {
						...
						if (!wwwAuth.contains("Kerberos") && (wwwAuth.contains("NTLM") || (wwwAuth.contains("Negotiate")))) {
							...
							newRealm = realmBuilder.setUri(URI.create(request.getUrl()).getPath()).setMethod(request.getMethod()).setUsePreemptiveAuth(true).parseWWWAuthenticateHeader(wwwAuth.get(0)).build();
						}
					...
					if (future.getNettyRequest().getMethod().getName().equals(Method.CONNECT.getName()) && statusCode == 200) {
					...
					if (!future.getAndSetStatusReceived(true) && updateStatusAndInterrupt(handler, status)) {
						...
						if (response.getContent().readableBytes() != 0) {
							updateBodyAndInterrupt(future, handler, new ResponseBodyPart(future.getURI(), response, NettyProvider.this, true));
						}
						finishUpdate(future, ctx, false);
						return;
					}

					if (nettyRequest.getMethod().getName().equals(Method.HEAD.getName())) {
						updateBodyAndInterrupt(future, handler, new ResponseBodyPart(future.getURI(), response, NettyProvider.this, true));
						markAsDone(future, ctx);
						drainChannel(ctx, future, future.getKeepAlive(), future.getURI());
					}
					...
					if (handler != null) {
						if (chunk.isLast() || updateBodyAndInterrupt(future, handler, new ResponseBodyPart(future.getURI(), null, NettyProvider.this, chunk, chunk.isLast()))) {
							if (chunk instanceof DefaultHttpChunkTrailer) {
								updateHeadersAndInterrupt(handler, new ResponseHeaders(future.getURI(), future.getHttpResponse(), NettyProvider.this, (HttpChunkTrailer) chunk));
							}
	...
	private final class WebSocketProtocol implements Protocol {
		...
		public void handle(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
			...
			if (e.getMessage() instanceof HttpResponse) {
				...
				HttpResponseStatus s = new ResponseStatus(future.getURI(), response, NettyProvider.this);
				HttpResponseHeaders responseHeaders = new ResponseHeaders(future.getURI(), response, NettyProvider.this);
				...
				s = new ResponseStatus(future.getURI(), response, NettyProvider.this);
				...
				if (frame.getBinaryData() != null) {
					...
					ResponseBodyPart rp = new ResponseBodyPart(future.getURI(), null, NettyProvider.this, webSocketChunk, true);
				...
				Debug.error("Invalid attachment {}", ctx.getAttachment());
			}
		}
